...
const VerseCard: React.FC<{ item: VerseArchiveItem }> = ({ item }) => (
  <Link to={`/verses/${item.id}`} className="block bg-white dark:bg-[#141417] border border-gray-300 dark:border-white/10 rounded-lg overflow-hidden shadow-sm hover:border-gray-400 hover:shadow-md transition-all">
    <div className="relative h-28 md:h-36 bg-gray-100 dark:bg-white/10 overflow-hidden">
    ...
    <div className="p-2.5 md:p-4">
      ...
      <h3 className="text-xs md:text-sm font-bold text-gray-900 dark:text-white uppercase leading-snug line-clamp-3 mb-1">
        "{item.text}"
      </h3>
      <p className="text-[11px] md:text-xs text-gray-400 dark:text-gray-500">{item.reference}</p>
      <div className="flex items-center justify-between pt-2 mt-2 md:pt-3 md:mt-3 border-t border-gray-100 dark:border-white/5">
        <span className="text-[10px] md:text-[11px] text-gray-400 dark:text-gray-500">{formatDate(item.displayDate)}</span>
        <span className="text-[10px] md:text-[11px] text-gray-400 dark:text-gray-500">{item.shareCount} shares</span>
      </div>
    </div>
  </Link>
);
...
const VerseCardSkeleton: React.FC = () => (
  <div className="bg-white dark:bg-[#141417] border border-gray-300 dark:border-white/10 rounded-lg overflow-hidden shadow-sm">
    <div className="h-28 md:h-36 bg-gray-300 dark:bg-white/10 animate-pulse" />
    <div className="p-2.5 md:p-4">
      <div className="h-3 w-16 bg-gray-300 dark:bg-white/10 rounded animate-pulse mb-2.5 md:mb-3" />
      <div className="h-4 w-full bg-gray-300 dark:bg-white/10 rounded animate-pulse mb-2" />
      <div className="h-4 w-2/3 bg-gray-300 dark:bg-white/10 rounded animate-pulse mb-3 md:mb-4" />
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 md:py-16">
        <div className="mb-4 md:mb-6">
          ...
          <AutoText as="h1" className="text-2xl md:text-4xl font-black uppercase tracking-tight text-gray-900 dark:text-white">All Verses</AutoText>
          <AutoText as="p" className="text-sm text-gray-500 dark:text-gray-400 mt-2">Browse every verse of the day from our archive.</AutoText>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4 mb-6 md:mb-8">
          <div className="flex items-center gap-2 md:gap-3 overflow-x-auto md:flex-wrap -mx-4 px-4 md:mx-0 md:px-0 pb-1 md:pb-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <AutoText as="span" className="text-[11px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500 mr-1 shrink-0">Filter</AutoText>
            {DAY_TABS.map(tab => (
              <button
                key={tab.label}
        ...
        {!hasLoaded ? (
          ...
          <AutoText as="p" className="text-gray-500 dark:text-gray-400">No verses found for this day.</AutoText>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
